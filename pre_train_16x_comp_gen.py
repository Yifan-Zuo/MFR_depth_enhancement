...
with tf.variable_scope("gen_down3_inten"):
    guided_16xto8x_gen=gslb.inten_downsample_unit(guided_8xto4x_gen, phase_train=phase_train)
...
with tf.variable_scope("gen_up4_dep"):
    dep_ten=gslb.LR_dep_fusion_unit(dep_ten, guided_16xto8x_gen, phase_train=phase_train)
    dep_ten=gslb.LR_dep_upsampling_unit(dep_ten, phase_train=phase_train)
...
#gen_reuse_vars = [v for v in var_list if (v.name.startswith("gen_dep")) or (v.name.startswith("gen_inten")) or (v.name.startswith("gen_down1_inten")) or (v.name.startswith("gen_up2_dep")) or (v.name.startswith("gen_down2_inten")) or (v.name.startswith("gen_up3_dep"))]
#saving_var_list=gen_reuse_vars+[v for v in var_list if (v.name.startswith("gen_down3_inten")) or (v.name.startswith("gen_up4_dep"))]
...
with h5py.File(train_h5F_addr,"r") as train_file:
    with tf.Session() as sess:
        sess.run(init_op)
        #gen_reuse_saver.restore(sess, "/home/yifan/eclipse4.6_workspace/guided_srgan_multi_skip_version/train/8x_pretrain_comp_gen/std5_version/full_model1/8x_comp_gen_model_full.ckpt-16")
        #saver_full.restore(sess, "/home/yizuo/Data/eclipse_neon3_workspace_new/guided_srgan_multi_skip_version/train/noise_free_models/models_16x/full_models2/16x_noisefree_gen_model_full.ckpt-7")
        for epo in range(epo_range):
            ...
            for ind in range(batch_total):
                ...
                if (ind+1)%3444==0:
                    save_path=saver_full.save(sess,"/media/kenny/Data/trained_models/residual_bn_csvt_models/noisy/16x/full_model1/16x_ny_full_model.ckpt",global_step=model_ind)
